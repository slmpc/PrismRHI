...
final class Gl41Support {
    private Gl41Support() {
    }

    static boolean hasLwjglOpenGl() {
        try {
            Class.forName("org.lwjgl.opengl.GL", false, Gl41Support.class.getClassLoader());
            return true;
    ...
    static GLCapabilities requireCapabilities() {
        try {
            ...
            if (!capabilities.OpenGL41) {
                throw new RhiException("The current OpenGL context does not expose OpenGL 4.1");
            }
            return capabilities;
        } catch (IllegalStateException exception) {
            throw new RhiException("OpenGL 4.1 backend requires a current LWJGL OpenGL context", exception);
        }
    }
    ...
    static int shaderType(RhiShaderStage stage, RhiShaderCodeType codeType) {
        if (codeType != RhiShaderCodeType.GLSL) {
            throw new RhiException("OpenGL 4.1 backend accepts GLSL shader modules only");
        }
        return switch (stage) {
            case VERTEX -> GL_VERTEX_SHADER;
            case FRAGMENT -> GL_FRAGMENT_SHADER;
            case COMPUTE -> throw new RhiException("OpenGL 4.1 backend does not support compute shaders");
        };
    }
    ...
    static void initializeTextureParameters() {
        org.lwjgl.opengl.GL11.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        org.lwjgl.opengl.GL11.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        org.lwjgl.opengl.GL11.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
        org.lwjgl.opengl.GL11.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    }
